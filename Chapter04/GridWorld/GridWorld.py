...
class Grid:
    def __init__(self, i, j, world_size):
        ...
        self.action = Action()
        # action up
        self.set_a(0, i == 0, [i - 1, j])
        # action down
        self.set_a(1, i == world_size[0] - 1, [i + 1, j])
        # action left
        self.set_a(2, j == 0, [i, j - 1])
        # action right
        self.set_a(3, j == world_size[1] - 1, [i, j + 1])

    def set_a(self, direction, condition, next_s):
        if condition:
...
class GridWorld:
    ...
    def show_value(self, accuracy, space):
        prefix = '%' + '%d' % (accuracy + space) + '.' + '%d' % accuracy + 'f|'
        split_line = '|' + ('-' * (accuracy + space) + '+') * (self.world_size[1] - 1) + '-' * (accuracy + space) + '|'
        print(split_line)
    ...
    def step(self, policy):
        ...
        for i in range(self.world_size[0]):
            for j in range(self.world_size[1]):
                if self.world[i][j].action.moves[0] == [-1, -1]:
                    continue
...
class OptimalPolicy(Policy):
    def cal_new_value(self, i, j, grids):
        ...
        for direction in range(4):
            ...
            value[direction] = -1 + self.discount * grids.value[
                new_pos[0], new_pos[1]]
        return np.max(value)
...
class RandomPolicy(Policy):
    def cal_new_value(self, i, j, grids):
        ...
        for direction in range(4):
            new_pos = grids.world[i][j].action.moves[direction]
            value += 0.25 * (-1 + self.discount * grids.value[
                new_pos[0], new_pos[1]])
        return value
